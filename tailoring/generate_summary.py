...
def generate_fit_summary_markdown(
    ...
    if tailored_resume.matched_skills:
        lines.append(
            f"- Matched skills already present in the master profile: {', '.join(tailored_resume.matched_skills[:8])}"
        )
    else:
        lines.append("- No direct skill matches were detected from the current master profile.")
    ...
    if tailored_resume.missing_keywords:
        lines.append(
            f"- Mentioned in the vacancy but not evidenced in the master profile: {', '.join(tailored_resume.missing_keywords[:8])}"
        )
    else:
        lines.append("- No major missing keywords were flagged from the extracted job signals.")
    lines.append("- The tailored resume only reorders and emphasizes existing profile content.")
    lines.append("- No skills, experience, achievements, or years were invented.")
    lines.append("")
    ...
    lines.append("- Updated the professional summary using only facts already present in the master profile.")
    lines.append("")
