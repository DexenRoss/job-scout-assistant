...
from tailoring.parse_master_resume import (
    ExperienceEntry,
    MasterResume,
    ProjectEntry,
    SkillCategory,
...
def tailor_resume_for_job(master_resume: MasterResume, job: JobPosting) -> TailoredResumeResult:
    job_text = _build_job_text(job)
    flattened_skills = _flatten_skill_items(master_resume.skills)
    matched_skills = [
    ...
    prioritized_experience = _prioritize_experience(master_resume, job_text, extracted_keywords)
    prioritized_projects = _prioritize_projects(master_resume, job_text, extracted_keywords)
    ...
    matched_resume_terms = {
        ...
        + _collect_technology_terms(master_resume)
    ...
    metadata = {
        ...
        "constraints": {
            "honesty_policy": (
                ...
                "in the master resume. No new experience, skills, or achievements were added."
            )
        },
    }
...
def _prioritize_skill_categories(
    ...
    *,
    ...
    for index, category in enumerate(categories):
        ...
        if category_score == 0:
            category_score = sum(1 for item in ordered_items if _phrase_in_text(item, job_text))
...
def _prioritize_experience(
    ...
    ranked_entries = [
        ...
        for index, entry in enumerate(master_resume.experience)
    ]
...
def _prioritize_projects(
    ...
    ranked_entries = [
        ...
        for index, project in enumerate(master_resume.projects)
    ]
    ranked_entries.sort(key=lambda item: (item[0], item[1]))
...
def _build_tailored_summary(
    *,
    ...
    summary_parts = [master_resume.summary.base.rstrip(".")]

    top_skills = matched_skills[:4]
    if top_skills:
        summary_parts.append(
            f"Relevant strengths already demonstrated in this profile include {', '.join(top_skills)}."
        )

    top_roles = [
        f"{entry.role} at {entry.company}"
        for entry in prioritized_experience[:2]
    ]
    if top_roles:
        summary_parts.append(
            f"Most relevant experience for roles like {job.title} includes {', '.join(top_roles)}."
        )
...
def _select_summary_highlights(
    *,
    ...
    ranked_highlights = _rank_text_fragments(
        master_resume.summary.highlights,
        job_text=job_text,
        extracted_keywords=extracted_keywords,
...
def _render_resume_markdown(
    *,
    ...
    if master_resume.education:
        ...
        for entry in master_resume.education:
    ...
    if master_resume.certifications:
...
def _collect_technology_terms(master_resume: MasterResume) -> list[str]:
    terms: list[str] = []
...
def _format_date_range(start_date: str, end_date: str | None, current: bool) -> str:
    if current:
